    ...
    return (
      <Login
        onAuthenticated={async (next) => {
          setSession(next);
          setAdmin(await isCmsAdmin(next));
        }}
      />
    );
  if (!admin) {
    ...
    <main className="grid min-h-screen place-items-center bg-[#f8f5ef] p-6">
      ...
      >
        ...
        <p className="mt-2 text-sm text-muted-foreground">
          Connectez-vous avec le même compte que sur /admin.
        </p>
...
function ContentEditor({
  ...
  async function load() {
    ...
    setError("");
    try {
      const [settingRows, mediaRows] = await Promise.all([
        getAdminSettings(session),
        getMediaAssets(session),
      ]);
      setSettings(settingRows);
      setMedia(mediaRows);
      const heroSetting = settingRows.find((row) => row.key === "home_hero") as
        SiteSetting<HomeHeroSetting> | undefined;
      const levelSetting = settingRows.find((row) => row.key === "school_levels") as
        SiteSetting<SchoolLevelsSetting> | undefined;
      setHero({
        ...defaultHero,
        ...(heroSetting?.value || {}),
        slides: heroSetting?.value?.slides?.length ? heroSetting.value.slides : defaultHero.slides,
      });
      setSchoolLevels({
        levels: levelSetting?.value?.levels?.length
          ? levelSetting.value.levels
          : defaultSchoolLevels.levels,
      });
    } catch (cause) {
  ...
  return (
    <main className="min-h-screen bg-[#f8f5ef] pb-16 text-foreground">
      <header className="sticky top-0 z-30 border-b border-primary/10 bg-white/95 backdrop-blur-xl">
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-4 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            <Link
              to="/admin"
              className="inline-flex items-center gap-2 rounded-full border border-primary/15 px-4 py-2 text-sm font-semibold text-primary"
            >
      ...
      <div className="mx-auto max-w-7xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
        {notice && (
          <p className="rounded-2xl bg-emerald-50 p-4 text-sm text-emerald-800">{notice}</p>
        )}
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-semibold tracking-[0.2em] text-gold uppercase">Accueil</p>
              <h2 className="mt-2 font-display text-3xl font-semibold text-primary">
                Carrousel principal
              </h2>
              <p className="mt-2 text-sm text-muted-foreground">
                Textes, boutons, vidéos et posters du haut de la page d'accueil.
              </p>
            </div>
            <button
              onClick={saveHero}
          ...
          <div className="mt-7 grid gap-5 md:grid-cols-2">
            ...
            <TextField
              ...
              value={hero.title || ""}
              onChange={(value) => setHero({ ...hero, title: value })}
            />
            <TextField
              label="Sous-titre"
              value={hero.subtitle || ""}
              onChange={(value) => setHero({ ...hero, subtitle: value })}
            />
            <TextField
              label="Description"
              value={hero.description || ""}
              onChange={(value) => setHero({ ...hero, description: value })}
            />
            <TextField
              label="Bouton principal"
              value={hero.primary_label || ""}
              onChange={(value) => setHero({ ...hero, primary_label: value })}
            />
            <TextField
              label="Lien bouton principal"
              value={hero.primary_url || ""}
              onChange={(value) => setHero({ ...hero, primary_url: value })}
            />
            <TextField
              label="Bouton secondaire"
              value={hero.secondary_label || ""}
              onChange={(value) => setHero({ ...hero, secondary_label: value })}
            />
            <TextField
              label="Lien bouton secondaire"
              value={hero.secondary_url || ""}
              onChange={(value) => setHero({ ...hero, secondary_url: value })}
            />
          </div>
        ...
        <section className="rounded-3xl bg-white p-6 shadow-sm sm:p-8">
          <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="mt-2 text-sm text-muted-foreground">
                Modifiez les photos, descriptions et points affichés sur les cartes.
              </p>
            </div>
            <button
              onClick={saveLevels}
          ...
          <div className="mt-7 grid gap-6 lg:grid-cols-2">
            {(schoolLevels.levels || []).map((level, index) => (
              <LevelEditor
                ...
                media={mediaOptions}
                onChange={(next) => {
                  const levels = [...(schoolLevels.levels || [])];
                  levels[index] = next;
      ...
      >
        ...
        {filtered.map((asset) => (
          <option key={asset.id} value={asset.storage_path}>
            {asset.alt_text || asset.storage_path}
          </option>
        ))}
      </select>
      {value && (
        <div className="mt-3 overflow-hidden rounded-xl border border-primary/10 bg-[#f8f5ef]">
          {kind === "video" ? (
            <video
              src={mediaPublicUrl(value)}
              className="h-36 w-full object-cover"
              muted
              controls
            />
          ) : (
            <img src={mediaPublicUrl(value)} alt="Aperçu" className="h-36 w-full object-cover" />
          )}
        </div>
      )}
    </div>
    ...
    <div className="rounded-2xl border border-primary/10 p-5">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <h3 className="font-semibold text-primary">Vidéo {index + 1}</h3>
        </div>
        <button onClick={onDelete} className="text-xs font-semibold text-red-600">
          Supprimer
        </button>
      </div>
      <div className="mt-4 grid gap-4 md:grid-cols-2">
        ...
        <TextField
          ...
          onChange={(value) => onChange({ ...slide, description: value })}
        />
        <label className="block">
          <span className="text-sm font-semibold text-primary">Durée affichée (sec)</span>
          <input
            className="input-admin mt-2"
            type="number"
            min={1}
            ...
            onChange={(e) => onChange({ ...slide, duration: Number(e.target.value) || 8 })}
          />
        </label>
        <div />
        <div>
          <p className="mb-2 flex items-center gap-2 text-sm font-semibold text-primary">
            <Video className="size-4" /> Vidéo
          </p>
          <MediaSelect
            value={slide.src}
            media={media}
            kind="video"
            onChange={(value) => onChange({ ...slide, src: value })}
          />
        </div>
        <div>
          <p className="mb-2 flex items-center gap-2 text-sm font-semibold text-primary">
            <ImageIcon className="size-4" /> Image d'aperçu
          </p>
          <MediaSelect
            value={slide.poster}
            media={media}
            kind="image"
            onChange={(value) => onChange({ ...slide, poster: value })}
          />
        </div>
      </div>
    </div>
...
function LevelEditor({
  ...
  level: SchoolLevelContent;
  media: MediaAsset[];
  onChange: (level: SchoolLevelContent) => void;
}) {
  return (
    <div className="rounded-2xl border border-primary/10 p-5">
      ...
      <div className="mt-4 space-y-4">
        <TextField
          label="Titre"
          value={level.title}
          onChange={(value) => onChange({ ...level, title: value })}
        />
        <label className="block">
          <span className="text-sm font-semibold text-primary">Description</span>
          <textarea
            className="input-admin mt-2 min-h-24"
            value={level.summary}
            onChange={(e) => onChange({ ...level, summary: e.target.value })}
          />
        </label>
        <div>
          <p className="mb-2 text-sm font-semibold text-primary">Photo</p>
          <MediaSelect
            value={level.image}
            media={media}
            kind="image"
            onChange={(value) => onChange({ ...level, image: value })}
          />
        </div>
        <label className="block">
          <span className="text-sm font-semibold text-primary">Points (un par ligne)</span>
          <textarea
            className="input-admin mt-2 min-h-28"
            value={level.points.join("\n")}
            onChange={(e) =>
              onChange({
                ...level,
